...
class FLIR_Dataset(Dataset):
    ...
    def __init__(self, root_dir, scale=4, transform=None, train=True, max_samples=1000, use_homography=True):
        ...
        # Kích thước các ảnh - ĐÃ SỬA THEO ĐÚNG KÍCH THƯỚC
        self.rgb_orig_h, self.rgb_orig_w = 1600, 1800  # RGB gốc: 1600x1800
        self.depth_h, self.depth_w = 128, 160          # Depth gốc: 128x160
        self.gt_h_orig, self.gt_w_orig = 512, 640      # GT gốc: 512x640
        ...
        depth_files = glob(os.path.join(self.depth_dir, '*.tiff')) + glob(os.path.join(self.depth_dir, '*.tif'))
        gt_files = glob(os.path.join(self.gt_dir, '*.tiff')) + glob(os.path.join(self.gt_dir, '*.tif'))
        
        print(f"\nFound files in {split}:")
        print(f"  RGB (1600x1800): {len(rgb_files)} files")
        ...
        if use_homography and len(self.rgb_files) > 0:
            if train:
                ...
                FLIR_Dataset.train_H = self.H  # Lưu lại để dùng cho val
                print(f"\n✅ Homography matrix computed for TRAIN:\n{self.H}")
            else:
                # Nếu là val, dùng homography từ train
                if FLIR_Dataset.train_H is not None:
                    self.H = FLIR_Dataset.train_H
    ...
    def apply_homography_and_resize(self, rgb):
        ...
        if self.use_homography and self.H is not None:
            ...
            rgb_warped = cv2.warpPerspective(
                rgb, 
                self.H, 
                (self.depth_w, self.depth_h),  # Target size = depth size (160,128)
